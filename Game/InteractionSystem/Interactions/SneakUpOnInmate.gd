...
func sneaked_up_on_text():
	...
	var someone_You_veThey_ve:String = "You've" if dom.isPlayer() else "They've"
	...
	if( abs(affectionValue) < 0.10 ):
		...
		possible.append_array([
			"A very familiar "+ dom_feline_canine_hybrid +" scent fills the air, but {sub.you} {sub.youVerb('exert')} {sub.yourselfThemself} trying to remember the scent's owner. {dom.youVerb('Were', 'Was')} "+ someone_youIt +" an ally? A traitor? {sub.YouHe} {sub.youAreHeIs} unable to tell.",
			"{sub.You} {sub.youVerb('hope')} to determine the assailant's identity from only their scent, but it proves to be difficult. There is a trace of.. "+ dom_feline_MAYBE +" smell in it? The associations it evokes in {sub.you} are "+ ( ( "positive, at least. But {sub.youHe} cannot trust "+ someone_yourTheir +" intentions." ) if(affectionValue > 0) else "somewhat negative. {sub.YouHe} {sub.youHeVerb('swallow')}." ),
		])
		...
		var dom_friendlyPeculiarStern:String = "friendly" if(domIsKind) else ( "stern" if(domIsMean) else "peculiar" )
		...
		var necessaryEnding:String = ""

		if( dom.isPlayerOwner() ):
			necessaryEnding = "{dom.YoureTheyre} {sub.yourHis} owner.."
		elif(!subIsNaive):
			necessaryEnding = "You've been on good terms so far, but not even the most dependable inmates have earned {sub.your} absolute trust, so {sub.youHe} {sub.youHeVerb('opt')} to remain careful."
		else:
