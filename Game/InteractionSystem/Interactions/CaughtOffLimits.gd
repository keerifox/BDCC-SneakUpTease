...
func shouldRunOnMeet(_pawn1, _pawn2, _pawn2Moved:bool):
	...
	if(_pawn1.isStaff() && _pawn2.isInmate()):
		...
		if(isBadLoc):
			var sneakUpOpportunityChance = getSneakUpOpportunityChance({guard=_pawn1, inmate=_pawn2})
			var hasSneakUpOpportunity = RNG.chance(sneakUpOpportunityChance)

			if(!hasSneakUpOpportunity):
				return [true, {guard=_pawn1.charID, inmate=_pawn2.charID}, {}]

			startInteraction("SneakUpOnGuard", {guard=_pawn1.charID, inmate=_pawn2.charID})
			return [false]

	return [false]

func getSneakUpOpportunityChance(_pawns:Dictionary):
	var inmate = _pawns["inmate"].getChar()
	var sneakUpOpportunityChance = 5

	if( isCharacterWearingStaffUniform(inmate) ):
		var guardPerceptionRatio = ( _pawns["guard"].scorePersonalityMax({PersonalityStat.Naive: -1.0}) + 1.0 ) / 2.0
		sneakUpOpportunityChance += ceil( 20 * (1.0 - guardPerceptionRatio) )

	if( !inmate.hasBoundLegs() ):
		sneakUpOpportunityChance += 15

	if( !inmate.hasBoundArms() ):
		sneakUpOpportunityChance += 5

	sneakUpOpportunityChance = clamp(sneakUpOpportunityChance, 0, 100)

	return sneakUpOpportunityChance

func isCharacterWearingStaffUniform(character:BaseCharacter):
	var charInventory = character.getInventory()

	if( !charInventory.hasSlotEquipped(InventorySlot.Body) ):
		return false

	var item = charInventory.getEquippedItem(InventorySlot.Body)
